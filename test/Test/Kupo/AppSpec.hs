...
import Kupo.App
    ( ChainSyncClient, mailboxSize, producer )
    ...
    prop "State-Machine" $ withMaxSuccess maxSuccess $
        forAll genInputManagement $ \inputManagement -> do
            ...
            forAllCommands stateMachine Nothing $ \cmds -> monadicIO $ do
                ...
                let chainProducer = newMockChainProducer queue
                let kupo = kupoWith tracers chainProducer `runWith` env
...
newMockChainProducer queue callback = do
    ...
    callback mailbox $ \_tr checkpoints _statusToggle -> do
        case checkpoints of
            ...
            _ -> do
                fail "Mock chain producer does not support starting from a list of \
                     \checkpoints. Indeed, the goal is to test arbitrary sequences \
                     \of execution, for which starting 'from scratch' is therefore \
                     \not only sufficient but necessary."
